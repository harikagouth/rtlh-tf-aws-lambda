...
module "lambda" {
  ...
  source_code_location       = var.source_code_location #The location of source code. It can be either s3 or local or ecr.
...
module "iam_role_policy_example" {
  ...
  iam_policy_names = ["example-policy-ec2", "example-policy-s3"]

  iam_role_policies = [
    file("${path.module}/policy_documents/policy_ec2_example.json"),
